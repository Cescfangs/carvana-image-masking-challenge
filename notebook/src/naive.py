...
def validation_get_optimal_thr():
    train_files = glob.glob(
        INPUT_PATH + 'train_masks/*.gif')[:NUM_OF_IMAGES_FROM_TRAIN]
    train_masks = []
    div_factor = 1

    avg_mask = np.zeros((1280 // div_factor, 1918 //
                         div_factor), dtype=np.float64)
    print('AVG Mask shape: {}'.format(avg_mask.shape))
    for f in train_files:
        mask = np.array(Image.open(f), dtype=np.uint8)
        if div_factor != 1:
            mask = cv2.resize(
                mask, (mask.shape[1] // div_factor, mask.shape[0] // div_factor), cv2.INTER_LINEAR)
