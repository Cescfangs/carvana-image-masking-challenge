...
def create_model(input_shape):
    ...
    conv1 = Conv2D(filters=3, kernel_size=3,
    ...
    deconv3 = Conv2DTranspose(
        3, 3, strides=4, activation='relu', padding='same')(conv3)
    deconv2 = Conv2DTranspose(
        3, 3, strides=2, activation='relu', padding='same')(conv2)
...
if __name__ == '__main__':
    ...
    for i, img_id in enumerate(ids_train[:num_train]):
        ...
        y_batch = np.array(
            [np.expand_dims(load_mask(img_id, j), 2) / 255.0 for j in imgs_idx])
        X.append(x_batch)
        y.append(y_batch)
        del x_batch, y_batch
        if i % 2 == 0:
    ...
    history = model.fit(X_train, y_train, epochs=1, validation_data=(
        X_val, y_val), batch_size=8, verbose=1)
